...
import { rutService, RutExtractionWithClientResponse, UploadProgress, RutDetails, JobStatusResponse } from "@/lib/api/rut-service"
...
export default function TercerosPage() {
  ...
  const handleStartExtraction = async () => {
    ...
    await rutService.extractRutWithJobPolling(
      selectedFiles,
              ...
              {isExtracting && (
                <div className="space-y-3">
                  <div className="flex justify-between text-sm text-brand-text-secondary">
                    <span>
                      {jobStatus
                        ? `Procesando archivos (${jobStatus.files_processed}/${jobStatus.total_files})`
                        : 'Iniciando procesamiento...'
                      }
                    </span>
                    <span>{jobStatus?.percentage ?? 0}%</span>
                  </div>
                  <div className="w-full bg-brand-bg-alt rounded-full h-2">
                    <div
                      className="bg-brand-indigo h-2 rounded-full transition-all duration-500"
                      style={{ width: `${jobStatus?.percentage ?? 0}%` }}
                    />
                  </div>
                  {jobStatus && jobStatus.files_processed > 0 && (
                    <div className="flex items-center gap-4 text-xs text-brand-text-secondary">
                      <span className="flex items-center gap-1">
                        <CheckCircle className="h-3 w-3 text-success" />
                        {jobStatus.successful} exitosos
                      </span>
                      {jobStatus.failed > 0 && (
                        <span className="flex items-center gap-1">
